...
def build_chrome_args(realtime_enabled: bool, camera_file: str = "") -> list[str]:
    """Build media flags, optionally using a Y4M file as the camera feed."""
    args = [
        "--use-fake-ui-for-media-stream",
        "--disable-blink-features=AutomationControlled",
    ]
    if not realtime_enabled or camera_file:
        args.insert(1, "--use-fake-device-for-media-stream")
    if camera_file:
        args.append(f"--use-file-for-fake-video-capture={camera_file}")
    return args
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    ...
    camera_file = os.environ.get("MEETING_RECORDER_CAMERA_IMAGE", "").strip()
    chrome_args = build_chrome_args(rt["enabled"], camera_file)
    ...
    try:
        with sync_playwright() as pw:
            ...
            from meet_runner import enforce_camera_state
            ...
            from meet_runner import EmptyMeetingGuard, detect_call_ended, enforce_camera_state, participant_count, send_recording_announcement
            empty_guard = EmptyMeetingGuard(
                confirmations=int(os.environ.get("MEETING_RECORDER_EMPTY_CONFIRMATIONS", "3"))
            )
            while not stop_flag["stop"]:
                ...
                if not state.in_call and (now - last_admission_check) > 3.0:
                    ...
                    if admitted:
                        ...
                        enforce_camera_state(page, in_call=True)
                        if not announcement_sent:
                            announcement_sent = send_recording_announcement(page)
                ...
                if state.in_call and (now - last_camera_check) > 2.0:
                    last_camera_check = now
                    enforce_camera_state(page, in_call=True)

                if state.in_call and (now - last_participant_check) > float(
                    os.environ.get("MEETING_RECORDER_PARTICIPANT_POLL_SECONDS", "10")
                ):
                    last_participant_check = now
                    if detect_call_ended(page):
                        state.set(leave_reason="removed_or_call_ended")
                        break
                    if empty_guard.observe(participant_count(page)):
                        state.set(leave_reason="everyone_else_left")
                        break
            ...
            # Save rotated Google cookies/local storage so authenticated Meet
            # sessions remain valid longer between interactive logins.
            if auth_state:
                from meet_runner import save_refreshed_auth_state
                save_refreshed_auth_state(context, auth_state)
            context.close()
            browser.close()
